...
contract EchidnaTest {
    ...
    // =========================================================================
    function _invariant_totalIdleAndDeployedValueMatchesAccounting() internal view {
        TranchePool tp = handler.tranchePool();
        uint256 left = tp.getTotalIdleValue() + tp.getTotalDeployedValue();
        uint256 right = tp.getTotalDeposited() - tp.getTotalLoss() + tp.getTotalRecovered();
        assert(left == right);
    }
    ...
    function _invariant_OutStandingPrincipalMatchesDeployed() internal view {
        assert(handler.outstandingPrincipal() == handler.tranchePool().getTotalDeployedValue());
    }
    
    // =========================================================================
    // INVARIANT 3: Total Unclaimed Interest + Idle Value = Token Balance
    // =========================================================================
    function _invariant_totalUnclaimedInterestAndIdleValueMatchesTotalTokenBalance() internal view {
        TranchePool tp = handler.tranchePool();
        uint256 left = tp.getTotalUnclaimedInterest() + tp.getTotalIdleValue() + tp.getProtocolRevenue();
        uint256 tokenBalance = ERC20Mock(address(handler.usdt())).balanceOf(address(tp));
    ...
    // =========================================================================
    function _invariant_totalDeployedValueMatchesSumOfIndividualTranches() internal view {
        TranchePool tp = handler.tranchePool();
    ...
    function _invariant_systemLevel_PrincipalIntegrity() internal view {
        ...
        for (uint256 i = 1; i < nextId; i++) {
            LoanEngine.Loan memory loan = le.getLoanDetails(i);
    ...
    function _invariant_lossRecoveryWaterfallSymmetry() internal view {
        ...
        if (tp.getTotalRecovered() >= tp.getTotalLoss()) {
            ...
            uint256 expectedShortfall = tp.getTotalLoss() - tp.getTotalRecovered();
